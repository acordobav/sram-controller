...
vcs -full64 -licqueue +incdir+$UVM_HOME/src $UVM_HOME/src/uvm.sv $UVM_HOME/src/dpi/uvm_dpi.cc -CFLAGS -DVCS '-timescale=1ns/1ps' '+vcs+flush+all' '+warn=all' '-sverilog' '+define+S60' '+define+VERBOSE' '+define+VCS' '-debug_access+all' '-cm' 'line+tgl+assert' '+plusarg_save' '+UVM_TESTNAME=test_basic' design.sv testbench.sv  && ./simv +vcs+lic+wait  ; echo 'Creating result.zip...' && zip -r /tmp/tmp_zip_file_123play.zip . && mv /tmp/tmp_zip_file_123play.zip result.zip